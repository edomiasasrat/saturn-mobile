...
export default function LoansPage() {
  ...
  const totalOwed = useMemo(() => loans.reduce((s, l) => s + l.remaining_amount, 0), [loans]);
  ...
  const handleAdd = async () => {
    ...
    setSubmitting(true);
    try {
      await addLoan({ person_name: name.trim(), phone_number: phone.trim() || null, original_amount: parsed, memo: memo.trim() || null });
      setAddOpen(false);
      resetAdd();
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "var(--bg)", paddingBottom: 96 }}>
      ...
      <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 8 }}>
        {loading ? (
          ...
          <p style={{ textAlign: "center", color: "var(--muted)", padding: "64px 0", fontSize: 15 }}>
            {search ? "No loans match your search." : "No loans yet. Tap + to add one."}
          </p>
        ) : (
          filtered.map((loan) => {
              ...
              <div key={loan.id} style={{
                ...
                >
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0, display: "flex", alignItems: "center", gap: 8 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 15, color: isPaidOff ? "var(--green)" : "var(--amber)" }}>
      ...
      {/* Add Loan Sheet */}
      <BottomSheet open={addOpen} onClose={() => !submitting && setAddOpen(false)} title="Add Loan">
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            <label style={lbl}>Person Name *</label>
            <input type="text" placeholder="Who owes you?" value={name} onChange={(e) => setName(e.target.value)} style={inp} />
          </div>
          <div>
          ...
          <div>
            <label style={lbl}>Amount Owed (ETB) *</label>
